...
public class RunBuildToolsTask implements Task {
    ...
    public RunBuildToolsTask(PrintStream outStream, Path projectPath) {
    ...
    private boolean validateEmptyOptionsToml(Tool.Field toolType, ClassLoader toolClassLoader) throws IOException {
        ...
        if (!requiredFields.isEmpty()) {
            for (String field: requiredFields) {
                TomlDiagnostic diagnostic = getTomlDiagnostic(toolType, field);
                this.outStream.println(diagnostic);
            }
            return true;
        }
    ...
    private TomlDiagnostic getTomlDiagnostic(Tool.Field toolType, String field) {
        ...
        DiagnosticInfo diagnosticInfo = new DiagnosticInfo(
                ...
                DiagnosticSeverity.ERROR);
        return new TomlDiagnostic(toolType.location(), diagnosticInfo, message);
    }
