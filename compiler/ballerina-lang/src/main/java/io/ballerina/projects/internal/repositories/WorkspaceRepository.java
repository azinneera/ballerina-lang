...
public class WorkspaceRepository extends AbstractPackageRepository {
    ...
    protected List<PackageVersion> getPackageVersions(PackageOrg org, PackageName name, PackageVersion version) {
        if (this.workspace.packages() == null || this.workspace.packages().isEmpty()) {
            // The packages will be null for langlib builds
            return Collections.emptyList();
        }
        if (version == null) {
            return this.workspace.packages().stream().filter(pkg -> pkg.packageOrg().equals(org)
                    && pkg.packageName().equals(name)).findFirst().map(pkg ->
                    Collections.singletonList(pkg.packageVersion())).orElse(Collections.emptyList());
        }
        return this.workspace.packages().stream().filter(pkg -> pkg.descriptor()
                .equals(PackageDescriptor.from(org, name, version))).findFirst().map(project ->
                Collections.singletonList(project.packageVersion())).orElse(Collections.emptyList());
    }
    ...
    protected DependencyGraph<PackageDescriptor> getDependencyGraph(
        ...
        for (Package pkg : this.workspace.packages()) {
    ...
    public boolean isPackageExists(PackageOrg org, PackageName name, PackageVersion version) {
        ...
        if (version == null) {
            return this.workspace.packages().stream().anyMatch(pkg -> pkg.packageOrg().equals(org)
                    && pkg.packageName().equals(name));
        }
        return this.workspace.packages().stream().anyMatch(pkg -> pkg.descriptor()
    ...
    public Collection<ModuleDescriptor> getModules(PackageOrg org, PackageName name, PackageVersion version) {
        ...
        if (version == null) {
            return this.workspace.packages().stream().filter(pkg -> pkg.packageOrg().equals(org)
                    && pkg.packageName().equals(name)).findFirst().map(pkg ->
                    pkg.moduleDependencyGraph().getNodes()).orElse(Collections.emptyList());
        }
        return this.workspace.packages().stream().filter(pkg -> pkg.descriptor()
                .equals(PackageDescriptor.from(org, name, version))).findFirst().map(pkg ->
                pkg.moduleDependencyGraph().getNodes()).orElse(Collections.emptyList());
    }
    ...
    public Optional<Package> getPackage(ResolutionRequest request, ResolutionOptions options) {
        ...
        if (request.version().isEmpty()) {
            return this.workspace.packages().stream().filter(pkg -> pkg.descriptor().equals(
                    request.packageDescriptor())).findFirst();
        }
        return this.workspace.packages().stream().filter(pkg -> pkg.descriptor().equals(
                request.packageDescriptor())).findFirst();
    }
